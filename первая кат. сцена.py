    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:  # Проверка нажатия мыши
                click_sounds[current_image_index].play()  # Воспроизведение соответствующего звука
...
def main():
    ...
    sound_paths = ["звуки шагов.mp3", "door-bello.mp3", "open-door.mp3", "шелест бумаги.mp3", "фоновая.mp3",
                   "фоновая.mp3", "фоновая.mp3", "фоновая.mp3", "фоновая.mp3"]
    click_sounds = load_sounds(sound_paths)

    background_music = pygame.mixer.Sound("фоновая.mp3")  # Фоновая музыка
    texts = [
        "",
        "",
